...
| Prompt | 做什么 |
|--------|--------|
| 按最新政策核对执行价并出处置提案（hero） | 对照最新政策事实核存量执行价：检出漂移建复核任务；命中激活规则的自动处置；其余转人审；人审结论沉淀为规则候选 |
...
1. 打开 `/`，看实时工作台统计；点 hero prompt「按最新政策核对执行价并出处置提案」，深链进 `/workspace`，演示数据源自动接入，agent 当场跑。
