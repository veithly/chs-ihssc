...
V2.3 针对评委实测体验补了三件事：**提案卡进对话流**（agent 跑完直接在回答下方出可编辑的修复提案卡 + 处置任务卡，改值→采纳即回写数据集、当场批准任务，不用去右侧点来点去）、**政策文档本地上传**（内网无外网时上传 PDF/CSV/XLSX/DOCX 落 artifact 留痕，CSV 自动解析价格事实建议、一次人审批量确认生效）、**批次页可达**（顶栏「批次队列」+ 工作台数据源直达 `/release/[id]` 批次详情）。
...
1. 打开 `/`，看实时工作台统计；点 hero prompt 进 `/workspace`，演示数据源自动 attach，agent 当场跑；回答下方直接出「处置提案卡」——改一条修复值点「采纳并回写」，批准一条任务，全部就地生效并留痕。
2. 右侧「政策事实」tab 点 `政策变更演示 640→560`（或「上传政策文件」传本地 CSV/PDF → 人审确认），重跑 hero prompt。
...
- Pitch+Demo video: `docs/video/pitch-demo-2.mp4`（1920×1080 / 30fps / 167s ≈ 2:47，pitch deck + 真实浏览器演示，按评审四维度组织）
- Video QA: `docs/video/pitch-demo/qa/summary.json`

## 内网部署（离线包）

- Windows 云桌面：`npm run package:win` → `dist-offline/jiaxu-win64-offline.zip`（含 Node 运行时，解压→填 `.env.local`→双击 `start.bat`），说明见 `docs/DEPLOYMENT_WINDOWS.md`
- Linux VM / 通用清单：`docs/DEPLOYMENT_OFFLINE.md`
...
- 政策文档本地上传（PDF/CSV/XLSX，SHA256 留痕 + CSV 事实解析）: `src/lib/policy/upload.ts`
- 修复提案决策（编辑/采纳回写数据集/忽略 + 决策留痕）: `src/lib/workspace/repairDecision.ts`
