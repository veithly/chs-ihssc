...
- schema_mapper: 校验价格批次字段是否完整且可标化（零售/网售渠道行豁免 item_code）。
- price_catalog_standardizer: 校验 item_code 是否命中医药价格目录，并识别高置信别名编码。
- reference_price_monitor: 对 unit_price 做参考价、最高有效价与涨幅监测（超限按红黄预警分档标注）。
- collective_landing_tracker: 跟踪集采中选价在 region / procurement_channel 下是否落地。
- retail_price_comparator: 机构价与零售药店/网售平台「即时达」集中价做 1.3 倍比对；零售无编码价按名称对应目录。
- spec_ratio_comparator: 同通用名不同包装按差比价公式（K=1.95^log₂X）折算可比价后比价。
...
- 机构价超零售「即时达」集中价 1.3 倍应进入异常处置；零售无编码价的名称↔编码对应必须人工确认。
- 差比价折算超限（2452号口径）属于需核验，督促企业调价是人的决定。
...
async function chatCompletionsCompat(
  ...
  if (!res.ok) {
    ...
    return { ok: false, status: res.status, errText, latency };
  }
  const data = (await res.json()) as ChatCompletionsData;
  return { ok: true, data, latency, jsonModeDropped };
}
...
export async function generateWorkspacePlan(
  ...
  try {
    const call = await chatCompletionsCompat(url, config.apiKey, body);

    if (!call.ok) {
      return {
        ok: false,
        category: call.status === 401 ? "auth_failed" : "provider_http_error",
        message: `Provider 返回 ${call.status}: ${call.errText.slice(0, 200)}`,
        meta: { model: config.model, baseUrlHost: host, latency_ms: call.latency },
      };
    }

    const { data, latency } = call;
